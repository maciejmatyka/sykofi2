...
//	mat[2] = roboczyO->b;    
	...
	if(roboczyO->punkt)
	{
			...
			{
				glColor3f(0,0,0);
			 	glutWireSphere(roboczyO->R-0.02,16,16);
...
//	 	glutWireSphere(roboczyO->R,24,24);
		glutSolidSphere(roboczyO->R-0.02,24,24);
	}
...
// os obrotu
	Punkt *P1=KorzenPunktow->prawy->prawy->prawy;
	Punkt *P2=KorzenPunktow->prawy->prawy->prawy->prawy;
	...
	glVertex3f(P1->r.x,P1->r.y+0.02,P1->r.z);
	glVertex3f(P1->r.x+no.x*10,P1->r.y+no.y*10+0.02,P1->r.z+no.z*10);
