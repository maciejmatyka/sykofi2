...
// puszczamy (pawia :) particla
	...
	for(int krowa=0;krowa<height;krowa+=DY)
	{
		...
		int k=0;
		while(k++<2000)// && px > 0 && py>0 && px<width-1 && py<height-1)
		{
			...
			px=px+InterpolatedU[(int)px][(int)py]*10.001;
			py=py+InterpolatedV[(int)px][(int)py]*10.001;
...
// puszczamy (pawia :) particla
	...
	for(int krowa2=0;krowa2<width-DX*2;krowa2+=DX*1.2)
	{
		for(int krowa=0;krowa<height-DY*2;krowa+=DY*1.2)
		{
			px=krowa2; //width/2+1;
			py=krowa;//height/4;
	
			int k=0;
			while(k++<6500)// && px > 0 && py>0 && px<width-1 && py<height-1)
			{
		...
		//		s=sqrt(pu*pu+pv*pv);
				...
				px=px+pu*0.05;//*10.001;
				py=py+pv*0.05;//*10.001;
...
// puszczamy (pawia :) particla
	...
	int ile=(height-DY*2)*(width-DX*2);
	...
	for(int krowa=0;krowa<ile;krowa+=DY)
	{
		...
		py=(rand()/(double)(RAND_MAX))*(height-DY*2);//height/4;
			...
			int k=0;
			while(k++<37500)// && px > 0 && py>0 && px<width-1 && py<height-1)
			{
...
*/
	...
	for(x=0;x<width-DX*2;x++)
		...
		colp *= (2225/UMAX);
			...
			V_Point(x,y, fabs(colu),fabs(colv) ,0);
...
void PlotOneSmoothField(int width, int height, int nx, int ny, double dx, double dy, double **f, int scale)
{
	...
	for(x=0;x<width-DX*2;x++)
		...
		colu *= (625/UMAX);
			...
			if(colu>0)
				V_Point(x,y, colu*0.9, colu*0.8, colu*0.7);
			else
...
//			colu=(fabs((u[i][j]+u[i+1][j])*(x01))/UMAX)*3255;
			...
			V_Point(x,y, fabs(colu),fabs(colv) ,0);//fabs(colp));
		}
...
void PlotVelocities(int width, int height, int nx, int ny, double dx, double dy, double **u, double **v, int scale)
{
	...
	for(j=0;j<ny-1;j++)
		for(i=0;i<nx-1;i++){
			...
			fU=fabs(centerU*centerU + centerV*centerV);
	...
	for(j=1;j<ny-1;j++)
		for(i=1;i<nx-1;i++){
			...
			fU=sqrt(fabs(centerU*centerU + centerV*centerV));

			// niecentralne na bokach
			DrawLine((i-0.5)*((double)width/(double)nx),((j)*((double)height/(double)ny)),
				(i-0.5)*((double)width/(double)nx)+(5*(centerU)*30),
				(j)*((double)height/(double)ny)+(5*(centerV)*30)
				,255,255,255);
	/*		DrawLine2((i-0.5)*((double)width/(double)nx),((j)*((double)height/(double)ny)),
				(i-0.5)*((double)width/(double)nx)+(14*(centerU)/fU),
				(j)*((double)height/(double)ny)+(14*(centerV)/fU)
				,155,155,155);
*/
