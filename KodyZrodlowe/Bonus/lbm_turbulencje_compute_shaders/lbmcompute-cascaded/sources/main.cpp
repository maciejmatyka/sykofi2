//
//Cascaded Lattice Boltzmann solver (Phys. Rev. E 73, 066705 (2006))
//code: Nicolas Delbosc (mnnd@leeds.ac.uk, Leeds) - Cascaded LBM implementation
//code: Maciej Matyka (http://www.matyka.pl/, maq@ift.uni.wroc.pl) - main, shaders, base lbm code
//03.2015 (Wroclaw)
//34
...
const int NX = 400;		// solver grid resolution
const int NY = 300;
char* filetobuf(std::string file); // (copied from Wikipedia)
...
float force = 0.00004;		// body force magnitude
/*--------------------- LBM State vector ----------------------------------------------------------------*/
...
void resetparticles(void)
{
	...
	for(i=0; i<NUMP; i++)
	{
		parGPU[i].x = 10;// (float)rand() / (float)RAND_MAX;
...
/*	for (int x = 0; x<NX; x++)
	...
	int cx = NX/5+(xMouse)*NX / 2.0;
	int cy = (yMouse)*NY / 2.0;

	int size = NX / 4;

	// garden base
	for (int xxx = NX/6+size*2.0; xxx < NX/6+size*2.0+size/3; xxx++)
	for (int yyy = 0; yyy < size/3; yyy++)
	{
		int idx = xxx + yyy*NX;
		if (idx >= NX*NY || idx<0)	break;
		F_cpu[idx] = 0;
		F_temp[idx] = 0;
	}
	...
	// house base
	for (int xxx = cx; xxx < cx + size; xxx++)
	for (int yyy = cy; yyy < cy + size; yyy++)
	{
		int idx = xxx + yyy*NX;
		if (idx >= NX*NY || idx<0)	break;
		F_cpu[idx] = 0;
	...
	// house roof
	for (int yyy = cy + size-NY/25; yyy < cy + size + size*0.75; yyy++)
	{
		float t01 = float(yyy - (cy + size)) / (size*0.75);
		for (int xxx = cx + int(t01*size*0.5); xxx < cx + size - int(t01*size*0.5); xxx++)
		{
			int idx = xxx + yyy*NX;
			if (idx >= NX*NY || idx<0)	break;
			F_cpu[idx] = 5;
	...
	// house chimney
	for (int yyy = cy + size; yyy < cy + size + size*0.65; yyy++)
	{
		for (int xxx = cx + size - size / 4; xxx < cx + size - size / 4 + size / 8; xxx++)
		{
			int idx = xxx + yyy*NX;
			if (idx >= NX*NY || idx<0)	break;
			F_cpu[idx] = 5;
	...
	// house door
	for (int yyy = cy; yyy < cy + size / 3; yyy++)
	{
		for (int xxx = cx + size / 2 - size / 10; xxx < cx + size / 2 - size / 10 + size / 5; xxx++)
		{
			int idx = xxx + yyy*NX;
			if (idx >= NX*NY || idx<0)	break;
			F_cpu[idx] = 1;
	...
	// house window
	for (int yyy = cy + size / 2; yyy < cy + size / 2 + size / 3; yyy++)
	{
		for (int xxx = cx + size / 6; xxx < cx + size / 6 + size / 6; xxx++)
		{
			int idx = xxx + yyy*NX;
			if (idx >= NX*NY || idx<0)	break;
			F_cpu[idx] = 1;
	...
	for (int yyy = cy + size / 2; yyy < cy + size / 2 + size / 3; yyy++)
	{
		for (int xxx = cx + size / 2 + size / 6; xxx < cx + size / 2 + size / 6 + size / 6; xxx++)
		{
			int idx = xxx + yyy*NX;
			if (idx >= NX*NY || idx<0)	break;
			F_cpu[idx] = 1;
...
//			else
//				F_temp[idx] = 1;
			if( sqrt(float((xx-x0)*(xx-x0)+(yy-y0)*(yy-y0))) < NX/14 )
				F_temp[idx] = 0;
		}	
	}*/
	glUnmapBuffer( GL_SHADER_STORAGE_BUFFER );	
...
void render(void) 
{
	...
	for (int x = 0; x < NX; x++)
	{
		for (int y = 0; y < NY; y++)
		{
			...
			if (F_cpu[idx] == 0)	c = 0.4;
			if (F_cpu[idx] == 5)	// roof
			{
				glColor4f(0.9, 0.1, 0.13, 1);
			}
			else
			glColor4f(c, c, c, 1);
