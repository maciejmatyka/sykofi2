...
//    Flag[NX/2] |= C_SRC;
 ...
 //   Flag[3*NX/4-4] |= C_SRC;
     ...
     // dołóż losowe
    int liczba =32;//rand() % 1000;
    for(int i=0; i<liczba; i++)
    {
//        Flag[i * NX/100] = C_SRC;
        int poz = rand() % NX;
        Flag[  poz  ] = C_SRC;
        srcF[ poz ]  = 0.02+(rand()/(float)(RAND_MAX)) * 0.001;
        srcA[ poz ]  = 0.014+(rand()/(float)(RAND_MAX)) * 0.001;
        srcP[ poz ]  = (rand()/(float)(RAND_MAX)) * 0.2*3.14;
    }
    t=0;
}
...
void drawwave(float SCALE)
{
    ...
    //	point1[PY]=U[i];
    //	point2[PY]=U[i+1];
        glVertex2f(point1[0],point1[1]+SCALE*U[i]);
        glVertex2f((point1[0]+(2/(float)NX)),point2[1]+SCALE*U[i+1]);
        point1[0]+=(2/(float)NX);
    }
    glEnd();
}
...
//			glColor3f(fabs(U[i]*2),fabs(U[i]*4),fabs(U[i]*8));
			...
			//glVertex3fv(point1);
			glVertex3f(1.0+point1[0], point1[1], point1[2]);
