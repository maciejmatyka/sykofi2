...
#define COLMAX 225 // max color value for rendering


/* Draw line function - very easy, but works, not important function */
...
*/
	...
	if (wrkpart->flag != P_AVAIL) {
		...
		&& wrkpart->y < height)
		V_Point((int) (wrkpart->x + 1),
			(int) (height - (wrkpart->y) -1)

/*			,120 + (int) ((wrkpart->y - mymin)*(210-120))/(mymax-mymin)
			,140 + (int) ((wrkpart->y - mymin)*(210-140))/(mymax-mymin)
			,180+ (int)  ((wrkpart->y - mymin)*(210-180))/(mymax-mymin));
*/
