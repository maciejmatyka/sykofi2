...
{
public:
	AnimSpring(Sprezyna *spring = NULL, float dt=0.01) : _myspring(spring), _dlugoscbak(spring->dlugosc), 
		_t(1), _dt(dt)
	{}
	...
	void StopAnim()
	{
		_myspring->dlugosc = _dlugoscbak;
	}

	void Anim()
	{
		// animuj _myspring->dlugosc
		_myspring->dlugosc = _dlugoscbak*sin(_t*3.14/2);
...
private:
	Sprezyna *_myspring;
	const float _dlugoscbak;		// pamietaj dlugosc default
	double _t;						// animacja (0-1)
	const float _dt;
};

 

	AnimSpring *PatrzWLewo;
...
// glowa - podloze
  	...
  	DodajSprezyne(KorzenSprezyn,12.0f,0,50.0f,P(4),P(7),S_OK|S_NRYS);	// w lewo (do nas)

// zapamietaj ta sprezyne
	for(Sprezyna *Robocza = KorzenSprezyn;Robocza->prawy != NULL; Robocza=Robocza->prawy)
	{

	}	// potrzebne
	PatrzWLewo = new AnimSpring(Robocza,0.001);
	
	DodajSprezyne(KorzenSprezyn,12.0f,0,50.0f,P(1),P(7),S_OK|S_NRYS);	// w prawo
...
// rozpatruj kolizje
	...
	// trzeba zrobic tak dla wszystkich obiektow do animacji (moze by tak liste jednokierunkowa?)
	PatrzWLewo->Anim();
