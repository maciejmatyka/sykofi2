...
double aspect,sx=1.3,sy=1.3,sz=1.3,anglex=0,angley=55,anglez=0;
...
#define lsz ( t * (roboczaS->punkt1->r.z) + (1-t) * (roboczaS->punkt2->r.z) )
	...
	if(!(roboczaS->flag & S_NRYS)) 
	{
	  ...
	  {
		for(float t=0; t<=1; t+=0.2)///roboczaS->dlugosc)
		{
...
void DrawVectors(void)
{
	...
	if(roboczyP->flaga & P_WEKT)
	{
		...
		RysujWektor(roboczyP->r,roboczyP->r+(roboczyP->v*dt),60.0);
...
void Draw(void)
{
	...
	const GLfloat lightAmbient[]={0.3f,1.1f,2.1f,0.5};
   const GLfloat lightDiffuse[]={1.0f,2.8f,1.3f,1.8};
